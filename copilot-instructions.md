# TreeTools R Package Development

TreeTools is an R package providing efficient implementations of
functions for the creation, modification and analysis of phylogenetic
trees. It includes C++ code for performance, comprehensive testing with
61+ test files, benchmarking infrastructure, and extensive CI/CD
workflows.
...
- **NEVER CANCEL**: R package builds can take 10-30+ minutes depending
  on dependencies and system performance. Set timeout to 60+ minutes.
...
- **NEVER CANCEL**: Full R CMD check takes 15-45+ minutes. Set timeout
  to 90+ minutes.
...
- **NEVER CANCEL**: Test suite has 61+ test files and can take 10-30+
  minutes. Set timeout to 60+ minutes.
...
- **NEVER CANCEL**: Build vignettes takes 5-15+ minutes. Set timeout to
  30+ minutes.
...
`ape` (\>= 5.0) - Phylogenetic analysis package - `Rcpp` (\>= 1.0.8) -
C++ integration - `Rdpack` (\>= 0.7) - Bibliography and citation support

## Validation

- Always run R CMD check for complete validation before finalizing
  changes.

- ALWAYS run the full test suite when modifying C++ code in src/
  directory.

- ALWAYS run lintr to ensure code style compliance before committing.

- For performance-critical changes, run benchmarks in benchmark/
  directory:
...
- Memory checking is available but optional (takes significant time):
  ...
  **Note**: Memory check scripts available in `memcheck/` directory:
  ...
  - `memcheck/all.R` - Run tests, examples, and build vignettes with
    memory checking

## Validation Scenarios

After making code changes, validate functionality by testing core
...
- **Full R CMD check**: 15-45+ minutes (NEVER CANCEL - set 90+ minute
  timeout)
- **Package build**: 10-30+ minutes (NEVER CANCEL - set 60+ minute
  timeout)
- **Vignette building**: 5-15+ minutes (NEVER CANCEL - set 30+ minute
  timeout)
...
- `src/` - C++ source code requiring compilation (17 C++ files,
...
- `.github/workflows/` - Extensive CI/CD with R-CMD-check, benchmarks,
  memory checks

### Important Files

- `DESCRIPTION` - Package metadata, dependencies, and system
  requirements
...
- `inst/_pkgdown.yml` - template for online documentation. (Check that
  new functions are registered here, ideally using `@family`)
...
- **R-CMD-check.yml**: Comprehensive checks on Windows, macOS, Ubuntu
  across R versions
- **benchmark.yml**: Performance regression testing triggered on PRs
- **memcheck.yml**: Memory checking with valgrind (runs `tests`,
  `examples`, `vignettes`)
...
- If package won’t build: check that all system dependencies are
  installed
