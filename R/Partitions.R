...
#' @return Returns an object of class `Splits`. two-dimensional array.  Columns correspond to unique
...
#' @export
Tree2Splits <- function (tr, asSplits = TRUE) {
  tr <- Cladewise(tr)
  splits <- cpp_edge_to_splits(tr$edge)
  # Return:
  if (asSplits) {
    nEdge <- dim(tr$edge)[1]
...
print.Splits <- function (x, details = FALSE, ...) {
  ...
  if (details) {
    ...
    for (i in seq_len(dim(x)[1])) {
      ...
      .DecodeBinary32(split[length(split)], stopAt = nTip %/% 32)
    }
  }
}
